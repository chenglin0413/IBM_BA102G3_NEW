...
public class RepmServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("getOne_For_Display".equals(action)) { // 來自select_page.jsp的請求
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					RequestDispatcher failureView = req.getRequestDispatcher("/front-end/restaurant/select_page.jsp");
				...
				String url = "/front-end/restaurant/listOneRepm.jsp";
		 ...
		 view)************/
		 req.setAttribute("repmVO", repmVO); // 資料庫取出的empVO物件,存入req
		 String url = "/front-end/restaurant/update_repm_input.jsp";
		 ...
		 .getRequestDispatcher("/front-end/restaurant/listAllRepm.jsp");
		 failureView.forward(req, res);
		 }
		 }
		...
		String repm_content = req.getParameter("repm_content").trim();
		 ...
		 try {
			 repm_startdate =
		 java.sql.Date.valueOf(req.getParameter("repm_startdate").trim());
...
//			repmVO.setRest_id(rest_id);
		 ...
		 RequestDispatcher failureView = req
		 .getRequestDispatcher("/front-end/restaurant/update_stpt_input.jsp");
		 failureView.forward(req, res);
		 return; //程式中斷
		 }
		 ...
		 /***************************2.開始修改資料*****************************************/
		 RepmService repmSvc = new RepmService();
		 repmVO = repmSvc.updateRepm(repm_name, repm_desc, repm_content, repm_startdate, repm_enddate, repm_status, repm_id);
		 ...
		 RequestDispatcher failureView = req
		 .getRequestDispatcher("/front-end/restaurant/update_repm_input.jsp");
		 failureView.forward(req, res);
		 }
		 }
		//
		if ("insert".equals(action)) { // 來自addEmp.jsp的請求
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					req.setAttribute("repmVO", repmVO); // 含有輸入格式錯誤的empVO物件,也存入req
					RequestDispatcher failureView = req.getRequestDispatcher("/front-end/restaurant/addRepm.jsp");
					failureView.forward(req, res);
					return;
				}
				...
				/*************************** 2.開始新增資料 ***************************************/
				RepmService repmSvc = new RepmService();
				repmVO = repmSvc.addRepm(rest_id, repm_name, repm_desc, repm_content, repm_startdate, repm_enddate, repm_status);

				/***************************
				 * 3.新增完成,準備轉交(Send the Success view)
				 ***********/
				String url = "/front-end/restaurant/listAllRepm.jsp";
